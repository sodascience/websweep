...
class Scraper:
    def __init__(self, save_html=True, max_level=3, base_path="data/scraped_data", classifier=lambda url,level: True, verify_ssl=False,
                       concurrency_per_domain=10, concurrency_websites=10):
        ...
        self.concurrency_websites = concurrency_websites
    ...
    async def __fetch_one_url(self, queue, kvk, level):
        """
        ...
        #Create path
        if url[-1] == "/":
            path = f"{self.base_path}/{kvk}/{urlparse(url).netloc}/{self.__get_current_date()}/{hash_url}_{url[:-1].split('/')[-1]}"
        else:
            path = f"{self.base_path}/{kvk}/{urlparse(url).netloc}/{self.__get_current_date()}/{hash_url}_{url.split('/')[-1]}"
        path = path.replace(" ","_")
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if status == 200:
                    #(TODO: This part should be in a different thread pool, it blocks the CPU and IO)
                    ...
                    urls = [urljoin(base_url, url_found) for url_found in urls if 
                            ("mailto:" not in url_found) and ("tel:" not in url_found)]
    ...
    async def __fetch_one_company(self, url, queue):
        """
        ...
        while (len(records) > 0) and (level < self.max_level):
            # fetch urls asynchroneously (producer is simple)
            for url in records:
                await queue.put(url)
            print(queue.qsize())
            # Create the tasks in parallel and consume them
            consumers = [asyncio.ensure_future(self.__fetch_one_url(queue, kvk=kvk, level=level)) 
                                                     for i in range(self.concurrency_per_domain)]
            ...
            for c in consumers:
                c.cancel()
            print("4")
            records = await asyncio.gather(*consumers) 
            print(url)
            print(records) 
    ...
    async def __fetch_all(self, records):
        ...
        #TODO use QUEue: https://stackoverflow.com/questions/52582685/using-asyncio-queue-for-producer-consumer-flow
        
        tasks = []
        async with ClientSession(headers=self.headers, trust_env=True, connector=TCPConnector(ssl=self.verify_ssl)) as self.session:
            for url in records:
                # Create one queue per company
                q = asyncio.Queue()

                task = asyncio.ensure_future(self.__fetch_one_company(url, q))
                tasks.append(task) 
