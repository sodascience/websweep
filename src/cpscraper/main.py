...
def scraper_address() -> None:
    ...
    """
    print(config.CONFIG_DIR_PATH)
    try:
        webbrowser.open('file:////{}'.format(config.CONFIG_DIR_PATH))
    except: 
        typer.secho(
...
def init(headless: bool = typer.Option(False, help="Run without GUI elements")) -> None:
    ...
    app_init_error = config.restore_app(folder)
    if app_init_error:
        typer.secho(
            f'Creating config file failed with "{ERRORS[app_init_error]}"',
            fg=typer.colors.RED,
...
def init(headless: bool = typer.Option(False, help="Run without GUI elements")) -> None:
    ...
    data_filename = typer.prompt("ENTER target folder name, BLANK for default", "scraper_data")
    ...
    while Path("{}/{}".format(folder, data_filename)).exists():
        ...
        data_filename = typer.prompt("ENTER alternative target folder name")
    ...
    app_init_error = config.init_app("{}/{}".format(folder, data_filename), ask_delete_files)
    if app_init_error:
        typer.secho(
            f'Creating config file failed with "{ERRORS[app_init_error]}"',
            fg=typer.colors.RED,
